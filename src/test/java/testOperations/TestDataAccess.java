...
public class TestDataAccess  {
    ...
    public TestDataAccess(EntityManager db) {
        this.db=db;
    }
    ...
    public Traveler createTestTraveler(String email) {
        if (org.mockito.Mockito.mockingDetails(db).isMock()) {
            return new Traveler(email, "Test Traveler", "1234");
        }
        try {
            db.getTransaction().begin();
            Traveler traveler = db.find(Traveler.class, email);
    ...
    public Ride createTestRide(String driverEmail, String from, String to, Date date, int nPlaces) {
        if (org.mockito.Mockito.mockingDetails(db).isMock()) {
            Driver driver = new Driver(driverEmail, "Test Driver", "1234");
            Car car = new Car("TEST-CAR-" + driverEmail, nPlaces, driver, false);
            driver.addCar(car);
            return driver.addRide(from, to, date, 0f, car);
        }
        try {
            db.getTransaction().begin();
            Driver driver = db.find(Driver.class, driverEmail);
    ...
    public Driver createTestDriver(String email) {
        if (org.mockito.Mockito.mockingDetails(db).isMock()) {
            return new Driver(email, "Test Driver", "1234");
        }
        try {
            db.getTransaction().begin();
            Driver driver = db.find(Driver.class, email);
