...
public class RateTravelerGUI extends JFrame {
	private Reservation r;
	private static final long serialVersionUID = 1L;
	
	private final JLabel jLabelEvents = new JLabel(ResourceBundle.getBundle("Etiquetas").getString("RateTravelerGUI.maintitle")); 
	...
	public RateTravelerGUI(Reservation r)
	{
		this.r=r;
		...
		jButtonClose.addActionListener(new ActionListener()
		{
			...
			{
				JFrame a = new DriverToRateGUI(r.getDriver());
				a.setVisible(true);
				dispose();
			}
		});
		...
		jButtonRate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(radioButtonOne.isSelected()) {
					facade.addRatingToTraveler(r.getTraveler().getEmail(), 1, r.getReservationCode());
				}else if(radioButtonTwo.isSelected()) {
					facade.addRatingToTraveler(r.getTraveler().getEmail(), 2, r.getReservationCode());
				}else if(radioButtonThree.isSelected()) {
					facade.addRatingToTraveler(r.getTraveler().getEmail(), 3, r.getReservationCode());
				}else if(radioButtonFour.isSelected()) {
					facade.addRatingToTraveler(r.getTraveler().getEmail(), 4, r.getReservationCode());
				}else if(radioButtonFive.isSelected()) {
					facade.addRatingToTraveler(r.getTraveler().getEmail(), 5, r.getReservationCode());
				}else {
